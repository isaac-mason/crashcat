 ...
 * one scaled-hull body per frame (the churn) then advances the sim. Seeding
 * matches cube-heap: warmup uses a distinct seed; the op rng is re-seeded at each
 * op boundary with `RNG_SEED + op`, so a labs op (op = 0) runs the fixed-seed
...
export function __sanity(): void {
    ...
    console.error(
        `[scaled-hulls] warmup ${(t1 - t0).toFixed(0)}ms  op ${(t2 - t1).toFixed(0)}ms  ` +
            `bodies ${count} scales ${NUMBER_OF_SCALES} (${SCALE_MIN}-${SCALE_MAX}x)  ` +
